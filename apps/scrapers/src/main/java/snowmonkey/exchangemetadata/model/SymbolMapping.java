    ...
    public static SymbolMapping create(String exchangeId) {
        try {
            ...
            for (JsonElement mapping : mappings) {
                String baseCurrency = mapping.getAsJsonObject().getAsJsonPrimitive("baseCurrency").getAsString();
                String baseId = mapping.getAsJsonObject().getAsJsonPrimitive("baseId").getAsString();
                currencyNameToId.put(baseCurrency.toUpperCase(), baseId.toUpperCase());

                String quoteCurrency = mapping.getAsJsonObject().getAsJsonPrimitive("quoteCurrency").getAsString();
                String quoteId = mapping.getAsJsonObject().getAsJsonPrimitive("quoteId").getAsString();
